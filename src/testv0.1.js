...
async function Find_from_token(){ // can be used for displaying the name of the user logged in
    if (localStorage.getItem("Admin_token")!=null){ // if an Admin_token is defined in the localstorage
        var Promise_pseudo = fetch('https://online-survey.app:3001/GET_Username?Token='+localStorage.getItem("Admin_token")+'',{ method: 'POST'})
        ...
        var Promise_pseudo = fetch('https://online-survey.app:3001/GET_Username?Token='+sessionStorage.getItem("Admin_token")+'',{ method: 'POST'})
        var Pseudo = await Promise_pseudo.then(response => response.json())
        return Pseudo; // retourne le pseudo correspondant
    }
    else{
...
// var username = Find_from_token(); disable for now
...
function Login(){
    ...
    // Send a post request
    xhttp.open("POST", "https://online-survey.app:3001/sign_in?name="+name+"&password="+password+"", true);
    xhttp.send(); 
...
function Register(){
    ...
    // Send a post request
    xhttp.open("POST", "https://online-survey.app:3001/sign_up?name="+name+"&password="+password+"&email="+email+"", true);
    xhttp.send(); 
}
else{
...
async function Submit_new_form(){
     ...
     // Send a post request
     xhttp.open("POST", "https://online-survey.app:3001/new_form?Admin_token="+Admin_token+"&Form_name="+Form_name+"&Content="+Content+"", true);
     xhttp.send(); 
